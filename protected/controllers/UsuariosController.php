...
class UsuariosController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('perfil','image','image128','imageProfile','impersonar'),
				'users'=>array('@'),
			),
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete','json'),
				'users'=>array('admin'),
			),
	...
	*/
	...
	{	
		if( Yii::app()->user->getState("rol") > 1 ){ // sólo si es administrador
	...
	private function _getUsuarios(){
		...
		$model = Usuarios::model()->with('empresa')->findAll();
	...
	private function _create(){
		...
		if($model->save()){
			...
			$notif->cuerpo = "<h1> Bienvenido </h1>
				<p> Apreciable ".$model->nombre.". </p>
				<p> Le damos la más coordial de las bienvenidas a nuestra aplicación web</p>
