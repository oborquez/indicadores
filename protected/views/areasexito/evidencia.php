...
<div class="page-header">
    ...
    <div class="row">
        ...
        <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-comment page-header-icon"></i>&nbsp;&nbsp; Evidencia de <?= $model->area->nombre?><br><?=date("d/m/y",strtotime($model->fecha1))?> - <?=date("d/m/y",strtotime($model->fecha2))?> </h1>
...
<script>
	...
	$(document).ready(function(){
		$("#save-comment").click(function(){
			...
			if(comment!=""){
				$.getJSON(baseUrl+"/areasexito/saveComment/"+<?echo $model->id?>,{comment:comment},function(r){
					if(r.status){
						$("#comment").val("");
						getComments();
						$("#cnl-comment").click();
...
<script type="text/javascript">
	...
	{
		$("#tbcomments").html("");
		$.get(baseUrl+"/areasexito/comentarios/<?echo $model->id?>",function(data){
		...
		{
			$("#files-body").html("");
			console.log(baseUrl+"/areasexito/getFiles/"+<?echo $model->id?>);
			$.getJSON(baseUrl+"/areasexito/getFiles/"+<?echo $model->id?>,function(r){
				$.each(r.files,function(k,item){
					var tr = $("<tr>").attr("file",item.id)
						.append( $("<td>").append( item.nombre))
						.append( 
							$("<td>")
							.append($("<a>").attr("href",baseUrl+"/areasexito/viewFile/"+item.id).addClass("btn btn-xs btn-primary").append($("<i>").addClass("fa fa-file")))
							.append(
								...
								.click(function(){
									if(confirm("En realidad desea eliminar el archivo?")){
										$.getJSON(baseUrl+"/areasexito/delfile/"+item.id,function(r){
											if(r.status){
				...
				<script>
					init.push(function () {
						$("#dropzonejs-example").dropzone({
							url: baseUrl+"/areasexito/uploadFiles/<?echo $model->id?>",
